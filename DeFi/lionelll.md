...
每个区块都把父区块的 hash 写进区块头。如果篡改了历史某个区块的内容（比如修改余额），该区块 hash 会改变，导致后续所有区块的 parentHash 都不再匹配。要让整条链“看起来合法”，攻击者必须重算从被篡改区块开始的所有后续区块，几乎不可能，这保证了链的不可篡改性。
...
问题 2：gasLimit 如何影响合约执行？
	...
	•	因此，gasLimit 是对区块总计算资源的限制，防止单块过重拖垮网络。
...
问题：什么是 ABI？input 如何被解析？
	...
	•	交易发送前，外部调用者会使用 ABI 把“函数 + 参数”编码成 input：
	•	先取函数签名（如 transfer(address,uint256)）做哈希，前 4 字节作为函数选择器；
...
通过查询 Receipt，我可以判断一笔交易是否成功、消耗了多少 Gas，以及合约触发了哪些事件。


扩展：
Merkle Tree 与轻节点验证
	•	区块内所有交易会被组织成一棵 Merkle Tree；
	•	根哈希 Merkle Root 被写入区块头；
	•	轻节点只保存区块头，当它需要验证某笔交易是否在某个区块中时：
	•	向全节点请求该交易的 Merkle Proof（从叶子结点到根的兄弟哈希路径）；
	•	在本地通过逐层哈希重建 Merkle Root；
	•	若计算出的 Root 与区块头中的 Root 一致，则证明该交易确实被包含在区块中。
